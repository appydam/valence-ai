  ...
  return (
    <div className="max-w-3xl mx-auto px-6 py-8 space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">Everything your AI ad team has done</p>
      </div>
        ...
        <div className="space-y-2">
          {adsTasks.map((task: {
            ...
            return (
              <div
                key={task._id}
                className="px-4 py-3 rounded-xl border border-border/50 bg-card hover:border-border/80 transition-colors"
              >
                <div className="flex items-start gap-3">
                  <Icon
                    className={`w-4 h-4 mt-0.5 shrink-0 ${statusCfg.color} ${task.status === "in_progress" ? "animate-spin" : ""}`}
                  />
                  <div className="flex-1 min-w-0">
                    ...
                    {/* Show deliverable preview if completed */}
