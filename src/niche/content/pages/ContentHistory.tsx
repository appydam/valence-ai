...
export function ContentHistory() {
  ...
  const contentTasks = (tasks ?? [])
    .filter((t: { tags?: string[] }) => t.tags?.includes("niche:content"))
  ...
  return (
    <div className="max-w-3xl mx-auto px-6 py-8 space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">Everything your AI content team has done</p>
      </div>
      ...
      {!isLoading && contentTasks.length === 0 && (
        <div className="text-center py-20">
          <Inbox className="w-10 h-10 text-muted-foreground/20 mx-auto mb-3" />
          <p className="text-sm text-muted-foreground">
            No activity yet. Ask your AI team to create something from the Home page.
          </p>
        </div>
      )}
