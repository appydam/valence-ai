...
export function NotificationBell({ collapsed }: { collapsed: boolean }) {
  const kazeCount = useQuery(api.notifications.countUnread, { agentName: "Kaze" }) ?? 0;
  const scoutCount = useQuery(api.notifications.countUnread, { agentName: "Scout" }) ?? 0;
  const forgeCount = useQuery(api.notifications.countUnread, { agentName: "Forge" }) ?? 0;
  const ghostCount = useQuery(api.notifications.countUnread, { agentName: "Ghost" }) ?? 0;

  const totalUnread = kazeCount + scoutCount + forgeCount + ghostCount;
  ...
  return (
    <Popover>
      ...
      <PopoverContent side="right" align="end" className="w-80 p-0 bg-card border-border max-h-[70vh] overflow-auto">
        ...
        {AGENTS.map(agent => (
          <AgentNotifications key={agent} agentName={agent} />
        ))}
        {totalUnread === 0 && (
...
function AgentNotifications({ agentName }: { agentName: AgentName }) {
  const notifications = useQuery(api.notifications.listForAgent, { agentName, unreadOnly: true }) ?? [];
    ...
    <div className="border-b border-border last:border-b-0">
      <div className="flex items-center justify-between px-3 py-2 bg-secondary/50">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-xs font-medium" style={{ color: `hsl(var(--agent-${config.color}))` }}>{agentName}</span>
